...
// clasp create-script --type sheets --title "tgBot_peruBot" --rootDir ./tgbot_gapps
...
Logger = useSpreadsheet('1xNbqY2rxb55TbSg02b4k3EclhvcFhlG03rSHEG2wglI'); 
...
function setWebhook() {
  ...
  var response = UrlFetchApp.fetch("https://api.telegram.org/bot" + TOKEN  + "/setWebhook?url=" + encodeURIComponent("https://script.google.com/macros/s/AKfycbyi6mzh0mhkcTNUvErDj5F3wzsuR0zhm0aO5eXyz_sTz5cQhaE2n8zjB_kybbSl2RZx/exec"));
    ...
    try {
      if (+timeout) {
        ...
        if (timeout < nowTime) {
          ...
          let actions = UserStates[getCellByUserId(id, "timeoutCallState").getValue()];
          doActions(fakeWebhook, actions);
          // changeStateTo("SENDING_DOC_2", ["WAIT_TIMEOUT1_7", "SENDING_DOC1_6", "WAIT_CALLBACK_CLAIM_5", "WANT_MATERIALS_4",
          // "WAIT_GET_MATERIALS_3", "READY_2", "WAIT_READY_1", "START_0"], id, true, fakeWebhook, undefined)
          // let currentState = getCellByRow(row, "stateFSM").getValue()
          // if (
          //   ["WAIT_TIMEOUT1_7", "SENDING_DOC1_6", "WAIT_CALLBACK_CLAIM_5", "WANT_MATERIALS_4",
          //   "WAIT_GET_MATERIALS_3", "READY_2", "WAIT_READY_1", "START_0"].some(allowedStateFrom => (currentState === allowedStateFrom))
          // ) {
          // }
        }
      }
...
function doActions(webhook, actions) {
  let chat_id = '0';
  if (webhook.message) {
    chat_id = webhook.message.from.id.toString()
...
function doPost2(e) {
  ...
  if (webhook.callback_query) {
    ...
    if (webhook.message.chat.id === idContorlChat) {
      chat_id = webhook.message.new_chat_member.id;
      getCellByUserId(chat_id, "stateFSM").setValue("SUCCES_15")
    }
  }
